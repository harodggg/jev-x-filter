 ...
 * - 只统计**不同账号**（同一账号重复不算农场）；
 * - 内容太短（<8 个有效字符）不参与，避免「哈哈」「太好了」这种大众短语造成误伤；
 ...
 * 太短（信息量不足）的文案不参与农场判定。
 *
 * 10 → 6 是**真站样本逼出来的**：X 标为「可能的垃圾信息」的两个账号发的是
 * `只入身体🥦🌰不入生活` / `只入身体🦵💪不入生活` —— 归一化（去 emoji）后都只有 8 个有效字符，
 * 旧阈值让这对教科书级的「同文案 + 各插垃圾字符」农场**整条不参与判定**。
 * 降到 8 之后仍然安全：2 个账号的农场命中还要再过 gate 的内容信号闸门
 * （adult/deceptive/solicitation/本地特征/预检诱饵），所以「两个人恰好发同一句寒暄」不会被隐藏；
 * ≥3 个账号才不需要内容信号。
 */
export const FARM_MIN_CHARS = 8;
