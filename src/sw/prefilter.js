...
export const STRONG_RULES = [
  ...
  },
  {
    ...
    // 独立验证发现的误伤：裸 `看福` 会命中「看福建舰 / 看福彩开奖」。钩子收紧成
    // 「给你看福(利) / 看福利 / 福利+资源类后缀」，`别拿…开玩笑` 只保留福利/资源（去掉「身体」）。
    pattern: /玩归玩闹归闹[\s\S]{0,16}(?:给你看福(?:利)?|看福利|福利(?:视频|资源|群|照|图))|(?:给你看福(?:利)?|看福利)[\s\S]{0,16}玩归玩闹归闹|别拿(?:福利|资源)开玩笑/,
  },
  {
    ...
    // 新闻语境下强规则会被降级成权重 1（见 matchRules），引用/讨论这类话术不会被误伤。
    // 独立验证发现的误伤：单独一个片段会误伤正常中文 ——
    //   「人到中年别只顾工作，不入生活…」（长辈式建议）、「针只进入身体的浅层组织」（医学描述）。
    // 所以**必须两个片段同时出现**（顺序不限）才算这句黑话；只写半句时由弱规则 `zh_body_weak` 兜 1 分。
    pattern: /只(?:入|进入|进)身体[\s\S]{0,12}不(?:进)?入生活|不(?:进)?入生活[\s\S]{0,12}只(?:入|进入|进)身体/,
  },
  {
...
export const WEAK_RULES = [
  {
    id: 'zh_body_weak',
    label: '疑似「只入身体」片段（单独不足以定罪）',
    sexual: true,
    // 组合规则未命中时（例如只写了半句），留一个 1 分弱信号：要配合乱码账号名等其它信号才成候选。
    pattern: /只(?:入|进入|进)身体|不(?:进)?入生活/,
  },
...
export const NEWS_CONTEXT = /警方|通报|法院|判决|起诉|整治|治理|专项行动|打击|查处|净网|未成年|举报|宣传|教育部|法规|律师|研究|报告|新闻|转载|辟谣|受害者|防范|警示|科普|科学|医学|医疗|医生|临床|理疗|治疗|患者|疾控|论文|记者|调查|报道|反诈|防骗|提醒|专家|教授|教程|揭露|套路/;
