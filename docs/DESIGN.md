...
- 需求：`认同` / `确定` / `哈哈哈` 这类**没有实质内容的附和**，同一线程只显示一条。
- 为什么不用模型：它们之间**字符串毫不相似**（3-gram 相似度≈0，农场也聚不起来），但按「同类空信息」看是同一件事；
  而这类回复量最大、价值最低 —— 为它们花一次调用不划算。所以用**本地词表 + 严格长度/结构判定**，0 成本。
- 保守边界：整串必须恰好等于某个附和短语（可带加强语与语气词），且归一化后 ≤12 字符；
  出现理由/转折/建议类词（因为/但是/其实/建议/为什么…）直接放弃；`不同意` / `不确定` 这类匹配不上任何核心，天然不会折叠。
- 归组：只在回复区（context 为 reply）且**有 threadId** 时按键折叠，同类才归一组，只留最早一条（duplicateOf 指向它）。
- 与其它层的关系：模型版 β 已经折叠时以模型为准；α 命中时不折叠（沿用既有优先级）；
