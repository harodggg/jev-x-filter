...
### 7.6.3 固定 10 类 + 全量文本覆盖率判定（v0.4.8）

用户第二轮反馈（又一条抽奖帖的 8 条回复一条都没折）并给定口径：
**「应该获得全量的信息，然后分类成几类。折叠几类就行了。最多 10 类。」**

- **为什么换思路**：v0.4.7 是「给每条截图配一个整串锚定句式」。这在真实评论区不可持续 ——
  每一批新样本（`中秋快乐梦想成真`、`我是真没有周边，咋搞`、`Okx的活动太高级了，周边也很漂亮`）
  都要新写一条正则。用户要的其实是**分类**：读全文 → 归入少数几类 → 折叠这几类。
- **类别固定 10 个**（`EMOTION_CLASSES`）：愤怒 / 喜悦 / 问候祝福 / 支持认同 / 反对 / 悲伤 / 赞美 /
  期待求取 / 参与 / 表情。合并口径：原 `confirmation` → 支持认同，原 `blessing` → 问候祝福，
  原 `social` → 期待求取。**类别顺序有意义**：`问候祝福` 必须排在 `支持认同` 之前，
  否则 `你好` 被填充词削成 `好` 会误判成「支持」（v0.4.5 的老坑）。
- **三级分类，逐级放宽**（`classifyEmotion`）：
  1. **核心短语**（≤ `LOW_SIGNAL_MAX_CHARS` = 12）：整串等于某个类别词，可带加强语/填充词/程度补语/重复；
  2. **整串锚定模板**（≤ `TEMPLATE_MAX_CHARS` = 20）：带称呼、宾语、自由前缀（自由前缀另做实质词过滤）；
  3. **覆盖率判定**（≤ `COVERAGE_MAX_CHARS` = 60）：把整条归一化文本从头到尾**贪心最长匹配**扫一遍，
     词表 = 各类别词（`words`）+ 中性词（`FILLER_LEXICON`：人称/助词/程度/语气/称呼/诉求动词，
     以及 `okx`、`活动`、`周边` 这类只在算覆盖率时中性的词）。
     判定：**覆盖率 ≥ `COVERAGE_MIN_RATIO`(0.7)** 且**某一类证据 ≥ 2 字** → 归入该类（多类取证据最多者，
     同分按类别表顺序）。`哇塞，参与啦，佳佳姐`、`中秋快乐，非常喜欢今年okx的周边，太爱了！`
     都是靠这一步归类，而不是专属句式。
- **否决闸门**（这条路的保护不靠长度，靠下面三道）：
  1. 含数字（`1`/`+1`/`111` 除外）或链接 → 不折叠；
  2. `SUBSTANTIVE`：因为/但是/其实/建议/数据/政策/规则/时间/价格/几点/放弃/太少/不划算… → 不折叠
     （所以 `这个活动名额太少，我放弃了`、`我没有时间参加这个活动`、`中秋月饼的销量数据` 都是 null）；
  3. `ADVICE_VETO`：劝告/否定句（`别`/`不要`/`不用`/`不必`/`请勿`/`记得`）→ 不折叠
     （`最好别来` 不是赞美）。
  另外**通用词刻意不放进类别词表**：`最好/最美/快乐` 只走锚定模板（否则 `今天天气最好` 会命中覆盖率路径），
  `没有/缺/需要/咋搞` 放进中性词表（否则 `我没有时间` 会被算成「期待求取」）。
- **成本不变**：全部本地、0 次模型调用；折叠仍是纯展示（不动 `band` / `accountAction`，不变量 I1）。
