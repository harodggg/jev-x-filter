...
## 7.5 农场阈值的边界（FARM_MIN_CHARS = 8）

- 农场（同文案多账号刷屏）用**归一化后的有效字符数**做下限：<8 不参与。阈值曾是 10，
  但真站样本（两个账号发同一句 8 字的 `只入身体…不入生活`，只在中间各插两个 emoji）正好卡在 8 ——
  旧阈值让这对教科书级农场**整条不参与判定**。降到 8 仍然安全：2 个账号的命中还要再过内容信号闸门
  （adult/deceptive/引流/本地特征/预检诱饵），「两个人恰好发同一句 6–7 字寒暄」不会隐藏；
  ≥3 个账号才不需要内容信号。
- 内容脚本里复制了同一套归一化（传统脚本不能 import），单测断言两边一致，防止漂移。

## 7.6 情绪言论分类与折叠（β 的本地分支）

- 需求：「情绪 认同 确定 之类的只显示一个」→「把所有的情绪言论给折叠/删除，然后给予愤怒，喜悦，支持，反对，之类的信息」。
- 为什么不用模型：情绪回复彼此**字符串毫不相似**（3-gram 相似度≈0，农场也聚不起来），量最大、信息最少；
  而它们又高度公式化（`生气`/`哈哈`/`支持`/`不同意`…），本地词表足够，0 成本。
- **7 类**：愤怒 / 喜悦 / 支持 / 反对 / 悲伤 / 确认 / 表情（纯 emoji）。
  保守边界：归一化后 ≤12 字符、整串必须恰好等于某个类别短语（可带加强语与语气词），
  出现理由/转折/建议/数据类词（因为/但是/其实/建议/数据/政策…）直接放弃。
  因此「我不同意，公开数据其实是反过来的」**不会**被当成情绪折叠 —— 折叠的是情绪，不是论点。
- **两种模式**（`semantics.emotion.mode`）：
  - `fold`：同线程同类情绪的**第一条**是代表条（`folded:false` + 页面挂「情绪 · 愤怒」徽标），其余折叠；
  - `hide`：**全部**折叠（用户说的「删除」），`folded:true`，条上写明类别。
- **两处范围**：`scope=thread`（回复区）按「同线程 + 同类」归组，首条是代表条；
  `scope=feed`（时间线/推荐流，没有线程语义）**每条各自折叠**并在条上写出原文预览 + 类别
  （用户截图里那批 `交朋友`/`美女啊`/`👍好`/`我也想去`/`Gm`/`太美了` 就是这一类）。
  时间线折叠受 `semantics.beta.foldInFeed` 控制、回复区受 `foldInReplies` 控制。
- 归组键是「同线程 + 同类别」，代表条用 **seq（观察顺序）** 判定而不是到达顺序 ——
  并发判定下后入窗的推文不会把先到的挤掉（有单测）。
- 与其它层的关系：模型版 β 已经折叠时以模型为准；α 命中时不折叠；
  **跳过判定的推文照样折叠** —— 「过短无媒体」跳过的是判定与账号动作，不该连带取消展示层整理。

### 7.6.1 独立验证发现的误伤与修法（v0.4.5）

- `不入生活` **单独出现不再定罪**：「人到中年别只顾工作，不入生活…」是正常中文表达。
  现在只有两个片段同现（顺序不限、允许「不进入生活」写法）才算那句黑话；半句由弱规则 `zh_body_weak` 兜 1 分。
- 钩子收紧：`看福` 会命中「看福建舰 / 看福彩开奖」→ 改成 `给你看福(利)?|看福利|福利+资源类后缀`。
- `NEWS_CONTEXT` 扩充 科普/医学/医生/临床/理疗/论文/记者/调查/反诈/防骗/提醒…：引用与医学描述会被降级而不是命中。
- 分类器过窄：连续加强语（`真好看`）要保留中间形态；程度补语（`难过死了`）要能剥离；
  填充词只削首/只削尾的中间形态也要试（`我服了` 曾被削成 `服`）。
- 类别顺序有意义：`问候` 必须排在 `支持` 之前，否则 `你好` 被削成 `好` 会误判成「支持」。

### 7.6.2 覆盖缺口与「合并成同一条」（v0.4.7）

用户在真站截图里圈出同一条活动帖下的 6 条回复，问「这些东西为什么不能折叠合并成同一条」：
...
- **实测根因（6/6 分类为 null，所以一条都没折）**：
  1. 短语表里没有「三连 / 参加 / 活动 / 来一份」这类**参与句**；
  2. 判定要求「整串恰好等于某条核心短语」，带称呼/宾语的赞美（`这个活动好啊`、`佳佳妹妹最好，最美`）落空；
  3. `希望` 在 `SUBSTANTIVE`（实质词）表里，`希望能中` 被当作「讲理由」直接放弃。
- **即使命中也不会合并**：旧归组键 `em:<threadId>:<类别>` 是**按类别**分组的，
  这 6 条落在赞美 / 期待 / 参与 3 组 → 3 条代表条 = 没合并。
- **修法 1：新增 `participation`（参与）类别**，并把「带前缀 / 带宾语 / 带称呼」的句式
  交给**整串锚定的模板**（`EMOTION_TEMPLATES`）。三条护栏保证不退化：
  1. 每条模板 `^…$` 整串锚定 + 固定小词表（不是「包含即命中」）；
  2. 归一化 ≤ `TEMPLATE_MAX_CHARS`(14) 且无数字/链接；核心短语表仍守 12 字线；
  3. `{ guard: 'prefix' }` 的模板允许 ≤4 字自由前缀（`好事多磨…`），但前缀命中
     `PREFIX_BLOCKED`（不/没/别/成本/数据/规则/为什么…）就放弃 → `成本太高可以来一份` 不会误折。
  模板在核心短语表之后、且**允许绕过 `SUBSTANTIVE`**（`三连了希望能中` 里的「希望」是模板的一部分）。
- **修法 2：整条线程合并成一个组**。`scope=thread` 的 `groupKey` 改为 `em:<threadId>:low`，
  `fold` 模式下只留**最早一条**代表条，其余不分类别全部折叠（这正是用户要的「合并成同一条」）。
  返回里新增 `merged`（多类/多条时为「低信息量附和」）、`classes`、`classLabels`、
  `classBreakdown`（如 `参与 3 · 赞美 2 · 期待 1`；排序=人数降序→类别表声明顺序）。
  `emotion` / `emotionLabel` **仍然报本条自己的类别**，保留 v0.4.4「给出愤怒/喜悦/支持/反对」的信息。
- **修法 3：详情页范围与主帖免疫**。`/status/<id>` 上所有 article 共用 threadId，而 X 现在常常
  不渲染「回复」文案（`getContext()` 判成 timeline → 退化成逐条折叠）；现在 `context==='reply'`
  **或** threadId 非空即按线程语义处理。同时 `id === threadId` 的主帖**永不折叠、也不能当代表条**
  （内容脚本给 `threadRoot`，纯函数再按 id 自证一次）。
- 时间线（`threadId` 为空）行为不变：没有线程语义处**每条各自折叠**、不跨帖合并。

## 7.7 规则的「review 下限」（reviewFloor）

- 有些黑话模板**本身就是强信号**，但文案里没有露骨词 —— 模型只看文案时会给 ordinary
  （实测 `玩归玩闹归闹…给你看福` → adult 0.03；`只入身体…不入生活` 在 0.52–0.94 之间波动）。
  这时「命中强规则 → 问模型 → 听模型的」会把真站样本放行。
- 机制：规则可以标 `reviewFloor: true`。命中且**未被新闻语境降级**（权重仍是 3）时，
  闸门给一个 **review 下限**（原因 `solicit_template`）：隐藏成待确认，不需要模型同意。
  它和「显示名即引流」「X 标垃圾分区」同一档 —— **绝不据此动账号**（I1 不变）；
  模型给出 hide/block 时仍然以模型为准（下限只抬不压）。
- 为什么安全：① 只在「短语组合」上命中（例如「玩归玩闹归闹」必须与「看福/福利」同时出现）；
  ② 新闻/讨论语境降级后下限失效（引用话术不会被隐藏）；③ 只隐藏、可一键「显示 / 误判」。
...
- **X 自己的分区标记也是信号**：X 会把可疑回复折叠在「可能的垃圾信息」标题下（真站截图里那条正文只有三个字）。
  内容脚本读这个标题（多语言表）→ 该分区里的推文**只隐藏成待确认**（`x_spam_section`），
  且正文过短（<4 字）也不再本地跳过。它是**结构信号不是内容证据**：永远不进账号动作，
  内容证据更强时（类别/成人概率/农场/图片）照旧给更强档位。
